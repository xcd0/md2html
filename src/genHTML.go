...
func getAtlUrl(str string) [][]string {
	...
	for _, v := range sp {
		...
		// strに`![]( "./img/a_1.png" )  `のように来たときはどうなるか
...
`, str,
		...
		if sAlt <= eAlt && eAlt <= sUrl && sUrl <= eUrl {
			// ファイルの拡張子ごとにヘッダをつける
			// gif,png,jpg,jpegのみ
			alt := v[sAlt:eAlt]
			url := v[sUrl:eUrl]
			...
			ext := filepath.Ext(url) // 一致した画像ファイルのパスから拡張子を調べる
			code := func() string {
				base64code := EncodeBase64(url)
				if ext == ".gif" {
				...
				log.Fatalf("Error : 対応していない画像形式です。: %s", url)
				return ""
			}()
			out = append(out, []string{alt, code, v[eUrl+1:]})
...
func ReplaceImg4mdPre(outputList []string, stringmd string) string { // {{{1
	...
	//log.Printf("outputList:%#v", outputList)
	...
	for _, line := range lines { // 一行ずつ
		if strings.Contains(line, "![") == false {
			...
			for _, imgAltUrl := range ret {
				tmp += fmt.Sprintf("![%s](%s)%s", imgAltUrl[0], imgAltUrl[1], imgAltUrl[2])
			}
			//log.Printf("%v", tmp)
			output += tmp + "\n"
